...
class mrla_base_layer(nn.Module):
    ...
    def forward(self, x, prev_K, prev_V):
        ...
        # hydra attention
        b,t,c,h,w=output_V.shape
        Q = Q / Q.norm(dim=-1, keepdim=True)
        K = K / K.norm(dim=-1, keepdim=True)
        kv = (K.view(b,t,c,1) * V.view(b,t,c,h*w)).sum(dim=1, keepdim=True)     #[b,1,c,h*w]
        out = Q.view(b,1,c,1) * kv
...
class mrla_module(nn.Module):
    ...
    def __init__(self, input_dim, init_cell=False, channel_wise=False):
        ...
        #LA_base_layer
        self.mrla = LA_base_layer(input_dim=input_dim, dim_perhead=self.dim_perhead, init_cell=init_cell) 
        self.init_cell = init_cell
