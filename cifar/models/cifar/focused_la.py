...
class linear_gla(nn.Module):
    ...
    def __init__(self, input_dim, feature_map=None, eps=1e-6, k_size=None, 
        ...
        self.positional_encoding = nn.Parameter(torch.zeros(size=(1, 1,input_dim)))
        self.scale = nn.Parameter(torch.zeros(size=(1, 1, input_dim)))
        self.focusing_factor = 2
...
class linear_la_layer(nn.Module):
    def __init__(self, input_dim, heads=None, dim_perhead=None, k_size=None, init_cell=False):
        ...
        self.positional_encoding = nn.Parameter(torch.zeros(size=(1,input_dim,1,1)))
        self.scale = nn.Parameter(torch.zeros(size=(1, 1, input_dim)))
        self.focusing_factor = 2
    ...
    def forward(self, x, prev_K, prev_V):
        ...
        Q = self.Wq(y) # Q: [b, 1, c] 
        K = self.Wk(K.view(-1,1,c)).view(b,-1,c) # k: [b, t, c]
...
class mrla_module(nn.Module):
    ...
    def __init__(self, input_dim, init_cell=False, channel_wise=False):
        ...
        if channel_wise:
            self.dim_perhead = 1
        self.mrla = linear_la_layer(input_dim, dim_perhead=self.dim_perhead,init_cell=init_cell) 
        self.init_cell = init_cell
