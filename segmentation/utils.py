...
class myNormalize:
    def __init__(self, data_name, train=True, mean=None, std=None):
...
class myCustomCrop:
    def __init__(self, top=0, left=0, height=256, width=256):
        self.top = top
        self.left = left
        self.height = height
        self.width = width
    ...
    def __call__(self, data):
        image, mask = data
        return (
            TF.crop(image, self.top, self.left, self.height, self.width),
            TF.crop(mask, self.top, self.left, self.height, self.width)
        )
