...
class mrla_base_layer(nn.Module):
    def __init__(self, input_dim, heads=None, dim_perhead=None, k_size=None, init_cell=False,layer_idx=None):
        ...
        self.k=2
        # Low-rank projection
        if  self.seq_len>self.k:
            self.proj_k = nn.Linear(layer_idx+1, 2)
            self.proj_v = nn.Linear(layer_idx+1, 2)
    ...
    def forward(self, x, prev_K, prev_V):
        ...
        if self.seq_len > self.k:
            # 调整低秩投影维度处理
            K = self.proj_k(K.permute(0, 2, 1)).permute(0, 2, 1)  # [b, k, c]
            V = self.proj_v(V.permute(0, 2, 3, 4, 1)).permute(0, 4, 1, 2, 3)  # [b, k, c, h, w]
 
        # 维度重组时添加投影维度处理
        K = rearrange(K, 'b t (g d) -> b g t d', g=self.heads) if self.seq_len <= self.k \
           else rearrange(K, 'b k (g d) -> b g k d', g=self.heads)
        
        V = rearrange(V, 'b t (g d) h w -> b g t d h w', g=self.heads) if self.seq_len <= self.k \
           else rearrange(V, 'b k (g d) h w -> b g k d h w', g=self.heads)
 
        # 优化后的注意力计算
        atten = torch.einsum('b g q d, b g k d -> b g q k', Q, K) * self._norm_fact
        atten = self.softmax(atten)
        
        # 调整V的展平方式
        V_flat = rearrange(V, 'b g k d h w -> b g k (d h w)')
        
        # 优化矩阵乘法表示
        output = torch.einsum('b g q k, b g k v -> b g q v', atten, V_flat)
...
class LA_base_layer(nn.Module):
    def __init__(self, input_dim, heads=None, dim_perhead=None, k_size=None, init_cell=False,layer_idx=None):
        super(LA_base_layer, self).__init__()
        self.input_dim = input_dim
        self.init_cell = init_cell
    ...
    def forward(self, x, prev_K, prev_V):
        ...
        if self.seq_len>self.k:
            K = self.proj_k(K.permute(0, 2, 1)).permute(0, 2, 1)  # (b, c, t)
            V = self.proj_v(V.permute(0,2,3,4,1)).permute(0,4,1,2,3)
...
class mrla_module(nn.Module):
    dim_perhead = 16  
    def __init__(self, input_dim, init_cell=False, channel_wise=False,layer_idx=None):
        super(mrla_module, self).__init__()
        if channel_wise:
            self.dim_perhead = 1
        #LA_base_layer  mrla_base_layer
        self.mrla = LA_base_layer(input_dim=input_dim, dim_perhead=self.dim_perhead, init_cell=init_cell,layer_idx=layer_idx) 
        self.init_cell = init_cell
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, init_cell=False, drop_path = 0.0,layer_idx=None):
        ...
        self.stride = stride
        self.mrla = mrla_module(input_dim= planes*self.expansion,init_cell=init_cell,layer_idx=layer_idx)
...
class ResNet(nn.Module):
    ...
    def _make_layer(self, block, planes, blocks, stride=1, init_cell=False, drop_path=0.0):
        ...
        layers.append(block(self.inplanes, planes, stride, downsample, init_cell, drop_path = drop_path,layer_idx=self.layer_idx))
        self.layer_idx+=1
        self.inplanes = planes * block.expansion
        for i in range(1, blocks):
            layers.append(block(self.inplanes,planes, drop_path = drop_path,layer_idx=self.layer_idx))
...
if __name__ == "__main__":
    # 构建模型
    model = linformer(depth=56, num_classes=100, block_name='bottleneck')
